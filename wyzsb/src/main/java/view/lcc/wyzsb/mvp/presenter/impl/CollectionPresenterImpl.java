...
public class CollectionPresenterImpl implements CollectionPresenter {
    ...
    private void loadData(final int page, final boolean get_data) {
        ...
        CollectionParams newsParams = new CollectionParams();
        newsParams.setPage(page);
        model.getCollection(newsParams, new ResultCallback<String>() {
            @Override
            ...
            public void run() {
                try {
                    ...
                    if (status == 1) {
                        ...
                        List<Collection> books = GsonUtils.fromJsonArray(result, Collection.class);
                        if (page == 1) {
